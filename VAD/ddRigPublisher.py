...
def validateNamingDetails(naming_details):
    '''
    validate correct information was entered
    '''
    print 'Validation Values:: ' + ' - '.join(naming_details.values())

    if naming_details['char_type'] != CHAR_TYPE_LABEL and \
        not naming_details['character'] in [CHAR_NAME_LABEL, '---'] and \
            not naming_details['rig_type'] in [RIG_TYPE_LABEL, '---']:

        if not naming_details['version'].isdigit():
            msg = 'Version is not a number.'
            validateNamingFailWin(msg)
            return False

        return True
    else:
        validateNamingFailWin()
        return False
